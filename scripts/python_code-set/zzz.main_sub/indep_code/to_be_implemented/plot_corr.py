...
def main():
    ...
    plot_corr_jkerr(CorrPS_jk, CorrSS_jk)
...
def plot_corr_jkerr(a_CorrPS, a_CorrSS):
    Ndata = len(a_CorrPS[:,0])
    
    print("#")
    for it in range(len(a_CorrPS[0,:])):
        if ((a_CorrPS[:,it] == 0).any()):
            Corr = np.zeros(Ndata)
        else:
            Corr = a_CorrPS[:,it] / a_CorrSS[:,it]
        print("%d %1.16e %1.16e %1.16e %1.16e %1.16e %1.16e" % 
              (it, 
               np.mean(a_CorrPS[:,it]), np.std(a_CorrPS[:,it]) * np.sqrt(Ndata-1),
               np.mean(a_CorrSS[:,it]), np.std(a_CorrSS[:,it]) * np.sqrt(Ndata-1),
               np.mean(Corr), np.std(Corr) * np.sqrt(Ndata-1)))
    print("#")
...
def set_args(ARGC, ARGV):
    global ifpathPS, ifpathSS, Bsize
