    ...
    for r in np.arange(0.0, np.max(ix), rbsize):
        ...
        if (nb != 0):
            ...
            if (nb == 1):
                ...
                oyerr = np.append(oyerr, np.sqrt(by2/float(nb)-(by/float(nb))**2) / np.sqrt(nb-1))
                oxerr = np.append(oxerr, np.sqrt(bx2/float(nb)-(bx/float(nb))**2) / np.sqrt(nb-1))
    
### Output results ###
    for ir in range(len(ox)):
        print("%lf %e %e %e" % (ox[ir], oy[ir], oyerr[ir], oxerr[ir]))
