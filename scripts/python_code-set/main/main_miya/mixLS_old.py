...
def fproc_wrapper(args):
    ...
    a_Edata      = args[6]
    a_Rmax       = args[7]
    results = solve_sch_diff_mixLS(a_ffuncs_V_C, a_params_V_C, a_ffuncs_V_T, a_params_V_T, 
                                   a_mass, a_Edata, a_Rmax)
...
### T-matrix calculation ###
    ...
    print("#\n# Calculate T-matrix...")
    if (Nproc == 1):
        Tmat = np.array([fproc_wrapper((iconf,
                                        fit_func_V_C, params_V_C[iconf,:], 
                                        fit_func_V_T, params_V_T[iconf,:], 
                                        mass, Edata, Rmax)) for iconf in range(Nconf)])
    else:
        args_procs = [(iconf,
                       fit_func_V_C, params_V_C[iconf,:],
                       fit_func_V_T, params_V_T[iconf,:], 
...
### Output the Phase shift & Calculation of the Scattering length ###
    ...
    ScatLength = np.array([solve_sch_diff_mixLS(fit_func_V_C, params_V_C[iconf,:],
                                                fit_func_V_T, params_V_T[iconf,:], 
                                                mass, np.array([1e-10]), Rmax)[0,:,:]
