    ...
    if (False): # For Debug
        for iconf in range(Nconf):
            for idata in range(Ndata):
                print("%lf" % xData[idata], end='')
                Mat = np.dot(Smat[iconf,idata,:,:], np.conjugate(Smat[iconf,idata,:,:].T))
                #Mat = Smat[iconf,idata,:,:]
                for ich in range(Nch):
                    for jch in range(Nch):
                        print(" %e %e" % (Mat[ich,jch].real, Mat[ich,jch].imag), end='')
                print("")
            print("\n")
        return 0
    
### Phase shfit calculation ###
    if   (Nch == 1):
        ...
        for idata in range(Ndata):
            print("%lf %e %e %e %e" % 
                  (xData[idata], *make_mean_err(Phase[:,0,idata]), *make_mean_err(Phase[:,1,idata])))
                   #*make_mean_err(np.array([calc_phase_Nch1(Smat[iconf,idata,0,0])[0]
                   #                         for iconf in range(Nconf)])),
                   #*make_mean_err(np.array([calc_phase_Nch1(Smat[iconf,idata,0,0])[1]
                   #                         for iconf in range(Nconf)]))))
        ...
        for idata in range(Ndata):
            print("%lf %e %e %e %e %e %e %e %e" % 
                   ...
                   *make_mean_err(MixAng[:,idata])   , *make_mean_err(Phase[0,:,1,idata])))
                   #*make_mean_err(np.array([calc_phase_Nch2(Smat[iconf,idata,:,:])[0][0]
                   #                         for iconf in range(Nconf)])),
                   #*make_mean_err(np.array([calc_phase_Nch2(Smat[iconf,idata,:,:])[0][1]
                   #                         for iconf in range(Nconf)])),
                   #*make_mean_err(np.array([calc_phase_Nch2(Smat[iconf,idata,:,:])[1]
                   #                         for iconf in range(Nconf)])),
                   #*make_mean_err(np.array([calc_phase_Nch2(Smat[iconf,idata,:,:])[2]
                   #                         for iconf in range(Nconf)]))))
    else:
        print("\nN.channel = %d has not implemented yet.\n" % Nch)
