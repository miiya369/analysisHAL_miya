...
namespace  { // Definitions of local variables (The initial parameters) & local functions for main part
   ...
   STATISTICS<ComplexField_XYZ> wave_1S0_2x2[3][2*2], wave_3S1_2x2[3][2*2];
   STATISTICS<ComplexField_XYZ> wave_1S0_1x1[3]     , wave_3S1_1x1[3];
   
   STATISTICS<ComplexField_XYZ> pot_eff_1S0_2x2[2*2], pot_eff_3S1_2x2[2*2];
   STATISTICS<ComplexField_XYZ> pot_eff_1S0_1x1     , pot_eff_3S1_1x1;
...
namespace  { // Inplementations of local functions for main part
   ...
   void input_NBSdata_from_bin(const int itime) {
      ...
      for (int dt=-1; dt<=+1; dt++) {
         ...
         for (int ich=0; ich<2; ich++) for (int jch=0; jch<2; jch++) {
            int  ijch = jch + 2*ich;
            wave_1S0_2x2[dt+1][ijch].input_data_bin(obase+"/"+odir_bin+"/NBSwave."+ch_2x2_str[ich]+"_"+
                                                    ch_2x2_str[jch]+".spin_0_.t"+time_dt_str+
                                                    ".bin_size"+Bsize_str);
            wave_3S1_2x2[dt+1][ijch].input_data_bin(obase+"/"+odir_bin+"/NBSwave."+ch_2x2_str[ich]+"_"+
                                                    ch_2x2_str[jch]+".spin_1_.t"+time_dt_str+
                                                    ".bin_size"+Bsize_str);
         }
         wave_1S0_1x1[dt+1].input_data_bin(obase+"/"+odir_bin+"/NBSwave."+ch_1x1_str+
   ...
   void potential_calculation(const int itime) {
      for (int ijch=0; ijch<2*2; ijch++) {
         pot_eff_1S0_2x2[ijch].mem_alloc(wave_1S0_2x2[0][ijch].Ndata());
         pot_eff_3S1_2x2[ijch].mem_alloc(wave_3S1_2x2[0][ijch].Ndata());
      }
      pot_eff_1S0_1x1.mem_alloc(wave_1S0_1x1[0].Ndata());
      pot_eff_3S1_1x1.mem_alloc(wave_3S1_1x1[0].Ndata());
      
      printf("Calculating potentials... \n");
      
#pragma omp parallel for
      for (int i=0; i<wave_1S0_2x2[0][0].Ndata(); i++) {
         ComplexField_XYZ tmp_pot_2x2[2*2], tmp_Rcor_2x2[3][2*2];
         ComplexField_XYZ tmp_pot_1x1     , tmp_Rcor_1x1[3];
         ...
         for (int ich=0; ich<2; ich++) for (int jch=0; jch<2; jch++) {
            ...
            for (int dt=-1; dt<=+1; dt++)
               tmp_Rcor_2x2[dt+1][ijch] = (wave_1S0_2x2[dt+1][ijch](i).rot_proj(ROT_REP_A1) /
         ...
         for (int ijch=0; ijch<2*2; ijch++) pot_eff_1S0_2x2[ijch](i) = tmp_pot_2x2[ijch];
         ...
         for (int dt=-1; dt<=+1; dt++)
            tmp_Rcor_1x1[dt+1] = (wave_1S0_1x1[dt+1](i).rot_proj(ROT_REP_A1) /
                                  (corr_1x1[0](i)(itime+dt) * corr_1x1[1](i)(itime+dt)));
         
         pot_eff_1S0_1x1(i) = Potential::get_potential_T2(tmp_Rcor_1x1[0], tmp_Rcor_1x1[1], tmp_Rcor_1x1[2],
                                                          tmp_mas_1x1_1, tmp_mas_1x1_2);
         
         for (int dt=-1; dt<=+1; dt++)
            tmp_Rcor_1x1[dt+1] = (wave_3S1_1x1[dt+1](i).rot_proj(ROT_REP_A1) /
                                  (corr_1x1[0](i)(itime+dt) * corr_1x1[1](i)(itime+dt)));
         
         pot_eff_3S1_1x1(i) = Potential::get_potential_T2(tmp_Rcor_1x1[0], tmp_Rcor_1x1[1], tmp_Rcor_1x1[2],
                                                          tmp_mas_1x1_1, tmp_mas_1x1_2);
   ...
   void output_XYZdata_to_text(const int itime) {
      ...
      for (int ich=0; ich<2; ich++) for (int jch=0; jch<2; jch++) {
         int  ijch = jch + 2*ich;
         pot_eff_1S0_2x2[ijch].output_data_err(obase+"/"+odir_pot+"/Pot."+ch_2x2_str[ich]+"_"+ch_2x2_str[jch]+
                                               ".Veff1S0.t"+time_str+".bin_size"+Bsize_str+".gnu",
                                               lattice_spacing, true);
         pot_eff_3S1_2x2[ijch].output_data_err(obase+"/"+odir_pot+"/Pot."+ch_2x2_str[ich]+"_"+ch_2x2_str[jch]+
                                               ".Veff3S1.t"+time_str+".bin_size"+Bsize_str+".gnu",
                                               lattice_spacing, true);
      }
      pot_eff_1S0_1x1.output_data_err(obase+"/"+odir_pot+"/Pot."+ch_1x1_str+".Veff1S0.t"+time_str+".bin_size"+
                                      Bsize_str+".gnu", lattice_spacing, true);
      pot_eff_3S1_1x1.output_data_err(obase+"/"+odir_pot+"/Pot."+ch_1x1_str+".Veff3S1.t"+time_str+".bin_size"+
                                      Bsize_str+".gnu", lattice_spacing, true);
   ...
   void output_XYZdata_to_bin(const int itime) {
      ...
      for (int ich=0; ich<2; ich++) for (int jch=0; jch<2; jch++) {
         int  ijch = jch + 2*ich;
         pot_eff_1S0_2x2[ijch].output_data_bin_reduce(obase+"/"+odir_pot+"/Pot."+ch_2x2_str[ich]+"_"+
                                                      ch_2x2_str[jch]+".Veff1S0.t"+time_str+".bin_size"+
      ...
      pot_eff_1S0_1x1.output_data_bin_reduce(obase+"/"+odir_pot+"/Pot."+ch_1x1_str+".Veff1S0.t"+time_str+
                                             ".bin_size"+Bsize_str+".bin", lattice_spacing, false);
      pot_eff_3S1_1x1.output_data_bin_reduce(obase+"/"+odir_pot+"/Pot."+ch_1x1_str+".Veff3S1.t"+time_str+
                                             ".bin_size"+Bsize_str+".bin", lattice_spacing, false);
      printf("Output potentials... end\n");
   }
}
